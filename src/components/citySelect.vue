		...
		<yd-navbar slot="navbar" :title="NavBar">
			...
			<mt-index-list>
				...
				<mt-index-section v-for='(item,index) in cityObj' :key="index" :index="item.type">
					<div v-for='(subItem,index) in item.items' :key="index" @click="cityChange(subItem.label,subItem.code)" style="height:1rem;border-top:1px solid #f8f8f8;line-height:1rem;padding-left:.3rem;">
